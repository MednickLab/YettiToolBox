...
%splits an array or matrix in 2. no error checking. use at own risk
    ...
    if cols==1 || rows==1
        part1 = arrayIn(1:ceil(length(arrayIn)/2));
        part2 = arrayIn(ceil(length(arrayIn)/2+1):end); 
    elseif rows > cols
        part1 = arrayIn(1:ceil(size(arrayIn,1)/2),:);
        part2 = arrayIn(ceil(size(arrayIn,1)/2+1):end,:);
    elseif cols >= rows
