...
% Please edit line 286 of DrawFormattedText to "yPosIsBaseline = 0;" to make later versions of Psychtoolbox compadable
    ...
    textPix = textSize*1.35; %this is the ratio from pix to size
    ...
    charsThatFits = round(x/textPix); %start from front, its faster...
    while(1)
        disp(textStripped(1:charsThatFits))
        [~,~,textBounds] = DrawFormattedText(ptb.win,textStripped(1:charsThatFits),'center','center');
        currentX = textBounds(3)-textBounds(1);
        errorPix = currentX - x;
        charsThatFits = round(charsThatFits-errorPix/1.35);
        if errorPix/1.35 < 1
            break
        end
    end
